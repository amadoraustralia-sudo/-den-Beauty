...
export default async function InicioPage({
  ...
  const { data: c1 } = await supabase
    .from("clientes").select("*").eq("auth_user_id", user.id).single();
  if (c1) {
    cliente = c1;
  } else if (user.email) {
    const { data: c2 } = await supabase
      .from("clientes").select("*").eq("email", user.email).single();
    cliente = c2;
    // Vincular auth_user_id se não tinha
    ...
    <div className="p-4 lg:p-8 max-w-2xl mx-auto lg:max-w-none">
      ...
      {toast === "bemvindo" && (
        <div className="mb-6 rounded-xl p-4 flex items-center gap-3" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0" }}>
          ...
          <p className="text-sm font-medium" style={{ color: "#15803d" }}>
            Bem-vinda ao Éden Beauty! Sua conta foi criada com sucesso. ✨
          </p>
        </div>
      )}
      ...
      <div className="mb-6">
        ...
        <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
          Bem-vinda ao seu espaço. Cuide-se com quem entende de beleza.
        </p>
      </div>
          ...
          <div className="rounded-xl p-4" style={{ background: "white", border: "1px solid var(--border)" }}>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm capitalize mt-0.5" style={{ color: "var(--text-muted)" }}>
                  {new Date(proximoAg.data + "T12:00:00").toLocaleDateString("pt-BR", { weekday: "long", day: "numeric", month: "long" })}
                </p>
              </div>
            ...
            {(proximoAg.status === "aguardando" || proximoAg.status === "confirmado") && (
              <div className="flex gap-2 mt-4 pt-4" style={{ borderTop: "1px solid var(--border)" }}>
                ...
                <Link
                  href={`/minha-conta/agendamentos/${proximoAg.id}/cancelar`}
      ...
      {servicos && servicos.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/agendar/novo" className="text-xs font-medium" style={{ color: "var(--brand-600)" }}>
              Ver todos →
            </Link>
          </div>
