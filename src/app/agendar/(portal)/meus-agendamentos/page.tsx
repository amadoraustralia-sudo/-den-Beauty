...
function AgCard({ a, showCancel, showRepeat }: { a: Agendamento; showCancel: boolean; showRepeat: boolean }) {
    ...
    <div className="p-4 lg:p-8 max-w-2xl mx-auto lg:max-w-2xl">
      ...
      <div className="flex rounded-xl p-1 mb-6" style={{ background: "white", border: "1px solid var(--border)" }}>
        {abas.map((t) => (
          <Link
            key={t.key}
            href={`/agendar/meus-agendamentos?aba=${t.key}`}
        ...
        <>
          {proximos.length === 0 ? (
            <div className="rounded-xl p-8 text-center" style={{ background: "white", border: "2px dashed var(--border)" }}>
              ...
              <Link href="/agendar/novo" className="btn btn-primary" style={{ fontSize: "0.8125rem", backgroundColor: "var(--brand-800)", borderColor: "var(--brand-800)" }}>
                Agendar agora
              </Link>
            </div>
          ) : (
