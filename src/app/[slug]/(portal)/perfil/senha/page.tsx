...
export default function AlterarSenhaPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (err) {
      ...
      router.push(`/${slug}/perfil?toast=senha`);
    }
  }

  return (
    <div className="p-4 lg:p-8 max-w-md mx-auto">
      <Link href={`/${slug}/perfil`} className="flex items-center gap-1.5 text-sm mb-6" style={{ color: "var(--text-muted)" }}>
      ...
      <div className="rounded-xl p-5" style={{ background: "white", border: "1px solid var(--border)" }}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="password" value={fields.nova} onChange={(e) => set("nova", e.target.value)}
          ...
          <div>
            ...
            <input type="password" value={fields.confirma} onChange={(e) => set("confirma", e.target.value)}
