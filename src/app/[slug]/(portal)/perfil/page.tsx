...
export default function PerfilPortalPage({
  ...
  const [saved, setSaved]       = useState(sp.toast === "senha");
  const [savedMsg, setSavedMsg] = useState(sp.toast === "senha" ? "Senha alterada com sucesso!" : "");
  const [error, setError]       = useState("");
  const [userEmail, setUserEmail] = useState("");
  const [clienteId, setClienteId] = useState<string | null>(null);

  const [fields, setFields] = useState({
    nome: "", telefone: "", data_nascimento: "", alergias: "", preferencias: "",
  });
    ...
    <div className="p-4 lg:p-8 max-w-xl mx-auto">
      ...
      {saved && (
        <div className="mb-6 rounded-xl p-4 flex items-center gap-3" style={{ background: "#f0fdf4", border: "1px solid #bbf7d0" }}>
          ...
          <p className="text-sm font-medium" style={{ color: "#15803d" }}>{savedMsg}</p>
        </div>
      )}
      {error && (
        <div className="mb-6 rounded-xl p-4" style={{ background: "var(--danger-bg)", border: "1px solid #fecaca" }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="rounded-xl p-5 space-y-4" style={{ background: "white", border: "1px solid var(--border)" }}>
          ...
          <div>
            ...
            <input value={fields.nome} onChange={(e) => set("nome", e.target.value)}
              placeholder="Seu nome" className="input" required />
          </div>
          <div>
            <label className="label">E-mail</label>
            <input value={userEmail} disabled readOnly className="input"
              style={{ opacity: 0.6, cursor: "not-allowed" }} />
            <p className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>O e-mail não pode ser alterado aqui.</p>
          </div>
          <div>
            <label className="label">WhatsApp</label>
            <input type="tel" value={fields.telefone} onChange={(e) => set("telefone", e.target.value)}
              placeholder="(11) 99999-9999" className="input" />
          </div>
          <div>
            <label className="label">Data de nascimento</label>
            <input type="date" value={fields.data_nascimento} onChange={(e) => set("data_nascimento", e.target.value)}
              className="input" />
          </div>
        </div>
        ...
        <div className="rounded-xl p-5 space-y-4" style={{ background: "white", border: "1px solid var(--border)" }}>
          ...
          <div>
            ...
            <textarea rows={3} value={fields.alergias} onChange={(e) => set("alergias", e.target.value)}
              placeholder="Ex: alergia a amônia, sensibilidade no couro cabeludo..."
              className="input" style={{ resize: "vertical" }} />
          </div>
          <div>
            <label className="label">Preferências</label>
            <textarea rows={3} value={fields.preferencias} onChange={(e) => set("preferencias", e.target.value)}
              placeholder="Ex: prefiro franja mais longa..."
              className="input" style={{ resize: "vertical" }} />
          </div>
        </div>
