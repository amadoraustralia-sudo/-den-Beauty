...
export default async function SalaoLandingPage({
  params,
}: {
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#F5F0E8" }}>
      ...
      <header style={{ backgroundColor: "var(--brand-800)", position: "sticky", top: 0, zIndex: 50 }}>
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {isLogado ? (
              <Link href={`/${slug}/inicio`} className="px-4 py-2 rounded-lg text-xs font-medium text-white" style={{ background: "rgb(255 255 255 / 0.15)" }}>
                Minha área →
              </Link>
            ) : (
              <>
      ...
      <section style={{ background: "linear-gradient(135deg, var(--brand-800) 0%, var(--brand-600) 100%)", paddingTop: "4rem", paddingBottom: "5rem" }}>
        <div className="max-w-5xl mx-auto px-4 text-center">
          ...
          <p className="text-lg mb-8 max-w-xl mx-auto" style={{ color: "rgb(255 255 255 / 0.7)" }}>
            Agende seus serviços em {nomeEstabelecimento} online, escolha seu profissional e horário — tudo sem sair de casa.
          </p>
          ...
          <Link
            href={isLogado ? `/${slug}/novo` : `/${slug}/cadastro`}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {profissionais.map((p) => (
                <div key={p.id} className="text-center p-4 rounded-xl" style={{ border: "1px solid var(--border)" }}>
                  <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-3 text-lg font-bold"
      ...
      <section className="py-16" style={{ background: "var(--brand-800)" }}>
        <div className="max-w-xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link href={isLogado ? `/${slug}/novo` : `/${slug}/cadastro`}
      ...
      {/* WhatsApp flutuante */}
      {config.telefone && (
        <a href={`https://wa.me/55${config.telefone.replace(/\D/g, "")}?text=Olá! Gostaria de agendar um horário no ${nomeEstabelecimento}.`}
