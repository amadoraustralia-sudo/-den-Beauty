-- ============================================================
-- 1. FIX RLS: profissionais_public_read só para role='cliente'
--    Admins que testaram o portal de outro salão (auto-criando
--    registro em clientes) não devem ver profissionais de lá.
-- ============================================================
...
-- ============================================================
-- 2. Atualiza get_horarios_disponiveis para respeitar bloqueios
--    de salão inteiro (profissional_id IS NULL = dia fechado).
--    Usado tanto para datas específicas quanto para feriados.
-- ============================================================
CREATE OR REPLACE FUNCTION public.get_horarios_disponiveis(
  ...
  p_duracao_total   integer DEFAULT NULL::integer
)
...
BEGIN
  ...
  -- Bloqueio de dia inteiro do salão (profissional_id IS NULL)
  IF EXISTS (
    SELECT 1 FROM horarios_bloqueados
    WHERE salao_id = p_salao_id AND profissional_id IS NULL AND data = p_data
  ) THEN RETURN; END IF;
  ...
  ELSE
    IF NOT EXISTS (
      SELECT 1 FROM configuracoes WHERE id = p_salao_id AND v_dia_key = ANY(dias_funcionamento)
    ) THEN RETURN; END IF;
  END IF;

  IF p_duracao_total IS NOT NULL AND p_duracao_total > 0 THEN
    v_duracao := p_duracao_total;
  ELSE
    ...
    BEGIN
      ...
      WHILE v_slot_time + (v_duracao || ' minutes')::interval <= v_prof_fechamento LOOP
        ...
        IF v_slot_dt >= v_now_limit THEN
          ...
          WHERE hb.data = p_data
            AND hb.hora_inicio < v_slot_fim
            AND hb.hora_fim    > v_slot_time
            AND (
              hb.profissional_id = v_prof.id
              OR (hb.profissional_id IS NULL AND hb.salao_id = p_salao_id)
            );
...
GRANT EXECUTE ON FUNCTION get_horarios_disponiveis(date, uuid, uuid, uuid, integer)
